...
String create_status_msg(String lat, String lon,  String _lock_status, int counter){
...
void loop() {
  ...
  if (millis() - lastPublishTime >= publishInterval && client.connected()) {
    if (currentWaypointIndex < totalWaypoints) {
      ...
      String payloadStr = create_status_msg(lat, lon, lock_status, msg_counter);
      char payload[128];
